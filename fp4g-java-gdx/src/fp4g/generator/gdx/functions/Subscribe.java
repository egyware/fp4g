...
public class Subscribe extends GdxFunction
{
	...
	throws CannotEvalException 
	{
		...
		if(to.length() > 0)
		{
			return new DirectCode(String.format("%s.addEventHandler(%s,%s)",to,onString,instance));
		}
		else
		{
			return new DirectCode(String.format("addEventHandler(%s,%s)",onString,instance));
		}
	}
